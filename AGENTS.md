...
- **Nothing about a user goes to our servers.** The app talks directly from the browser to Anthropic, OpenAlex, Wikipedia, arXiv, Dropbox and Google. Keep it that way: no proxy, and no analytics beyond Cloudflare Web Analytics, which Cloudflare injects into page loads (cookieless; its script is allowed in the CSP, `staticHtml` strips it from exports, and the privacy policy describes it). A new outside service also needs adding to the CSP in `scripts/headers.mjs`.
